...
一个用于学习 PyTorch 图像分类完整流程的固定长度验证码识别项目。项目使用
ImageNet 预训练的 DenseNet121，从整张图片直接预测 4 个字符，不进行字符切割。
...
```bash
uv run python train.py --loss multi-label --lr 0.001
```

训练 30 轮，每轮评估一次，只将验证准确率最高的权重保存到
`checkpoints/model-36.pth`；准确率相同时保留 loss 更低的权重。
...
```bash
# 只训练最后一层
uv run python train.py --classifier-only --loss multi-label --lr 0.001
...
| 路径 | 用途 |
| --- | --- |
| `datasets.py` | 从文件名构造标签并加载图片 |
| `models.py` | DenseNet121 和 144 维分类层 |
| `train.py` | 训练、验证和最佳权重保存 |
| `predict.py` | 单张识别与测试集逐张预测 |
| `main.py` | Tkinter 桌面界面 |
| `test_smoke.py` | 最小可运行检查 |
