...
`1×1` 卷积瓶颈。多随机种子实验后，当前推荐四卷积块和 8 通道瓶颈；代码保留面向
初学者的中文注释，方便观察各层张量形状与结构优化过程。
...
`--plot-curves` 默认关闭。第二阶段额外支持 `--bottleneck-channels` 和 `--seed`，其中随机
种子默认是 `0`。启用曲线后，每轮结束都会刷新训练集和验证集曲线；训练结束后，关闭
Matplotlib 窗口程序才会退出。
...
| `stage2_simple_cnn/models.py` | 带中文注释的三/四卷积块简单 CNN |
...
- 当前推荐配置为四卷积块、`128→8` 通道瓶颈、Dropout(0.1)、固定学习率 `0.001` 和
  轻量仿射数据增强；不启用 `ReduceLROnPlateau`，也不再继续搜索更多卷积块或更小瓶颈。
